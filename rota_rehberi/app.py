...
# 1. Klasör Yolunu Sabitleme (Görsel Sorununu Çözümü)
BASE_DIR = Path(__file__).parent

# 2. Sayfa ve Harita Ayarları
st.set_page_config(page_title="Bolu Turizm Rotaları", layout="wide")
...
rotalar = {
    ...
    "Kış Turizmi (Kartalkaya)": [
        ...
        {"isim": "Kındıra Köyü", "enlem": 40.6850, "boylam": 31.7550, "foto": "kindira.jpg", "sure": "25 dk", "mod": "4x4 Araç 🚙", "aktivite": "Köy Kahvaltısı"},
        {"isim": "Sarıalan (Kış)", "enlem": 40.6120, "boylam": 31.6500, "foto": "sarialan_kis.jpg", "sure": "15 dk", "mod": "4x4 Araç 🚙", "aktivite": "Kış Fotoğrafçılığı"},
        {"isim": "Kartalkaya Kayak Merkezi", "enlem": 40.6010, "boylam": 31.7950, "foto": "kartalkaya.jpg", "sure": "20 dk", "mod": "Kar Aracı ❄️", "aktivite": "Kayak ve Snowboard"}
    ]
}
...
if not st.session_state.giris:
    st.title("❄️ Bolu Tematik Rota Rehberi")
    st.info("Bolu'nun doğasını ve kış turizmini keşfetmek için giriş yapın.")
    if st.button("KEŞFETMEYE BAŞLA", use_container_width=True):
        st.session_state.giris = True
        st.rerun()
else:
    # 5. Yan Menü - Rota Seçimi
    st.sidebar.title("🗺️ Rota Seçimi")
    secilen_rota_adi = st.sidebar.selectbox("Görmek istediğiniz rotayı seçin:", list(rotalar.keys()))
    secilen_duraklar = rotalar[secilen_rota_adi]
    
    # Dinamik Navigasyon Linki (Seçilen rotaya göre ilk ve son durak koordinatları)
    start = secilen_duraklar[0]
    end = secilen_duraklar[-1]
    google_maps_link = f"https://www.google.com/maps/dir/{start['enlem']},{start['boylam']}/{end['enlem']},{end['boylam']}/"

    st.sidebar.markdown(f"""
        <a href="{google_maps_link}" target="_blank" style="text-decoration:none;">
    ...
    path_layer = pdk.Layer("PathLayer", pd.DataFrame([{"path": yol_noktalari}]), get_path="path", get_color=[30, 144, 255] if "Kış" in secilen_rota_adi else [211, 47, 47], width_scale=20, width_min_pixels=3)
    ...
    # 7. Durak Detayları ve Görsel Kontrolü
    st.markdown("---")
    for d in secilen_duraklar:
        with st.expander(f"📍 {d['isim']} - Detaylar", expanded=True):
            col1, col2 = st.columns([1, 1.5])
            with col1:
                foto_yolu = BASE_DIR / d['foto']
                if foto_yolu.exists():
                    st.image(str(foto_yol), use_container_width=True)
                else:
                    try:
                        st.image(d['foto'], use_container_width=True)
                    except:
                        st.warning(f"🖼️ {d['foto']} bulunamadı. Lütfen GitHub'da dosya adını kontrol edin.")
