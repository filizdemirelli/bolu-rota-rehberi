import streamlit as st
import pandas as pd
import pydeck as pdk
import os
from pathlib import Path

# 1. Sayfa Ayarları ve Hata Kontrolü
try:
    st.set_page_config(page_title="Bolu Rota Rehberi", layout="wide")
except:
    pass

# 2. Klasör Yolu Tanımı
BASE_DIR = Path(__file__).parent

# 3. PASTEL YEŞİL TEMA VE GRİ ÇUBUK DÜZELTME (CSS)
st.markdown("""
    <style>
    /* Ana Arka Plan */
    .stApp {
        background-color: #E8F5E9 !important;
    }

    /* Yan Menü (Sidebar) */
    section[data-testid="stSidebar"] {
        background-color: #E8F5E9 !important;
    }

    /* O Gri Çubuğu (Expander Başlığı) Açık Yeşil Yapma */
    .streamlit-expanderHeader {
        background-color: #C8E6C9 !important; /* Açık Yeşil */
        color: #1B5E20 !important; /* Koyu Yeşil Yazı */
        border: 1px solid #A5D6A7 !important;
        border-radius: 8px !important;
    }
    
    /* Panel İçeriği Arka Planı */
    .streamlit-expanderContent {
        background-color: #E8F5E9 !important;
        border: 1px solid #A5D6A7 !important;
    }

    /* Tüm Yazı Renklerini Sabitle */
    h1, h2, h3, h4, h5, h6, p, span, label, li {
        color: #1B5E20 !important;
    }

    /* Butonlar */
    div.stButton > button {
        background-color: #2E7D32 !important;
        color: white !important;
        border-radius: 8px;
        font-weight: bold;
    }
    </style>
""", unsafe_allow_html=True)

# 4. Rota Verileri
rotalar = {
    "EKOLOJİK KORİDOR (YAYLALAR)": [
        {"isim": "ŞEHİR OTELİ", "enlem": 40.7325, "boylam": 31.6082, "foto": "otel.jpg", "sure": "BAŞLANGIÇ", "mod": "YAYA", "aktivite": "Şehrin kalbinde konforla buluşun."},
        {"isim": "GÖLCÜK TABİAT PARKI", "enlem": 40.6552, "boylam": 31.6255, "foto": "golcuk_bolu.jpg", "sure": "20 DK", "mod": "ELEKTRİKLİ OTOBÜS", "aktivite": "Doğanın sessizliğini dinleyin."},
        {"isim": "SARIALAN YAYLASI", "enlem": 40.6120, "boylam": 31.6500, "foto": "sarialan.jpg", "sure": "15 DK", "mod": "MİNİBÜS", "aktivite": "Geleneksel lezzetleri keşfedin."},
        {"isim": "ALADAĞ YAYLALARI", "enlem": 40.5850, "boylam": 31.6350, "foto": "aladag.jpg", "sure": "10 DK", "mod": "BİSİKLET", "aktivite": "Kamp ve macera turizmi."}
    ],
    "KIŞ TURİZMİ (KARTALKAYA)": [
        {"isim": "BOLU MERKEZ", "enlem": 40.7350, "boylam": 31.6050, "foto": "merkez.jpg", "sure": "BAŞLANGIÇ", "mod": "VIP TRANSFER", "aktivite": "Kış masalı hazırlık noktası."},
        {"isim": "KINDIRA YAYLASI", "enlem": 40.6850, "boylam": 31.7550, "foto": "kindira.jpg", "sure": "25 DK", "mod": "4x4 ARAÇ", "aktivite": "Otantik yayla kahvaltısı."},
        {"isim": "SARIALAN (KIŞ)", "enlem": 40.6120, "boylam": 31.6500, "foto": "sarialan_kis.jpg", "sure": "15 DK", "mod": "4x4 ARAÇ", "aktivite": "Kış estetiği fotoğraf rotası."},
        {"isim": "KARTALKAYA KAYAK MERKEZİ", "enlem": 40.6010, "boylam": 31.7950, "foto": "kartalkaya.jpg", "sure": "20 DK", "mod": "KAR ARACI", "aktivite": "Kayak ve adrenalin keyfi."}
    ]
}

# 5. Giriş Sistemi
if 'giris' not in st.session_state:
    st.session_state.giris = False

if not st.session_state.giris:
    # GITHUB'DAN (YEREL KLASÖRDEN) GÖRSEL ÇAĞIRMA
    giris_foto = BASE_DIR / "yedigoller.jpg"
    if giris_foto.exists():
        st.image(str(giris_foto), use_container_width=True)
    else:
        # Eğer dosya henüz yüklenmediyse internetten bir yedek gösterir
        st.image("https://images.unsplash.com/photo-1570737197686-3974274c7d83?q=80&w=1200", use_container_width=True)
        st.warning("Not: 'yedigoller.jpg' GitHub'da bulunamadı, yedek görsel gösteriliyor.")
    
    st.title("BOLU TEMATİK ROTA REHBERİ")
    if st.button("KEŞFETMEYE BAŞLA", use_container_width=True):
        st.session_state.giris = True
        st.rerun()
else:
    # 6. Uygulama Paneli
    st.sidebar.title("MENÜ")
    secilen_rota_adi = st.sidebar.selectbox("BİR DENEYİM SEÇİN", list(rotalar.keys()))
    secilen_duraklar = rotalar[secilen_rota_adi]
    
    if st.sidebar.button("GİRİŞ EKRANINA DÖN"):
        st.session_state.giris = False
        st.rerun()

    st.title(secilen_rota_adi)

    # 7. Harita
    df = pd.DataFrame(secilen_duraklar)
    yol_noktalari = [[d["boylam"], d["enlem"]] for d in secilen_duraklar]
    line_color = [30, 136, 229] if "KIŞ" in secilen_rota_adi else [46, 125, 50]
    
    st.pydeck_chart(pdk.Deck(
        map_style="light",
        initial_view_state=pdk.ViewState(latitude=40.66, longitude=31.70, zoom=10),
        layers=[
            pdk.Layer("PathLayer", pd.DataFrame([{"path": yol_noktalari}]), get_path="path", get_color=line_color, width_scale=20, width_min_pixels=3),
            pdk.Layer("ScatterplotLayer", df, get_position="[boylam, enlem]", get_color=line_color, get_radius=300, pickable=True)
        ],
        tooltip={"text": "{isim}"}
    ))

    # 8. Detaylar (Açık Yeşil Çubuklar)
    st.markdown("---")
    for d in secilen_duraklar:
        with st.expander(d['isim'], expanded=True):
            c1, c2 = st.columns([1, 1.5])
            with c1:
                f_yolu = BASE_DIR / d['foto']
                if f_yolu.exists():
                    st.image(str(f_yolu), use_container_width=True)
                else:
                    st.info(f"Görsel Bekleniyor: {d['foto']}")
            with c2:
                st.write(f"**ULAŞIM:** {d['mod']}")
                st.write(f"**DENEYİM:** {d['aktivite']}")
